...
if(PHONE){
  ...
  if(synth&&nativeSpeak&&nativeCancel){
    ...
    synth.cancel=()=>{runtime.active=null;nativeCancel()};
  }
  ...
  function pauseNarration(){if(runtime.paused)return;runtime.paused=true;if(runtime.active&&nativeCancel)nativeCancel();renderControls()}
  ...
  pauseButton.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();runtime.paused?resumeNarration():pauseNarration()},true);
  ccButton.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();runtime.captionsOn=!runtime.captionsOn;renderControls()},true);
  ...
  if(film)new MutationObserver(placeCaption).observe(film,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});
  const captionNode=document.querySelector('#caption');
  if(captionNode)new MutationObserver(placeCaption).observe(captionNode,{childList:true,characterData:true,subtree:true});
  ...
  function begin(){
    ...
    const nativeBegin=document.querySelector('#begin');
    const voiceButton=document.querySelector('#voice');
    if(!nativeBegin&&!voiceButton){
      beginButton.disabled=false;
      beginButton.textContent='Tap to Retry';
      return;
    }
    ...
    shell.hidden=true;
    window.scrollTo(0,0);
    if(nativeBegin)nativeBegin.click();else voiceButton.click();
    requestAnimationFrame(placeCaption);
  }
  beginButton.addEventListener('click',begin);

  document.querySelector('#restart')?.addEventListener('click',()=>{runtime.restarting=true;nativeCancel?.();showLanding()},true);
  document.addEventListener('click',event=>{
    if(event.target.closest('#back,#next,[data-memory],[data-next],#route,[data-approve],[data-mandate]')){
      runtime.paused=false;runtime.active=null;nativeCancel?.();renderControls();
    }
  },true);
